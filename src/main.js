...
(function () {
  ...
  const filtersHtml = `<input type="radio" id="filter__all" class="filter__input visually-hidden" name="filter" checked/>
  ...
  const cardHtml = `<div class="card__form">
  ...
  addSection(sectionsPlace, `section`, filtersHtml, `main__filter filter container`);
  ...
  addSection(cardsPlace, `article`, editHtml, `card card--edit`);

  // add cards


  for (let i = 0; i < 3; i++) {
    addSection(cardsPlace, `article`, cardHtml, `card`);
  }
